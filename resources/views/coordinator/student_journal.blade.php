    ...
    <div class="w-full container mx-auto max-w-screen-xl mt-8  lg:px-12 px-2">
        <div class="min-h-[80vh] bg-white rounded-md border-0 shadow-md p-5 overflow-auto">
            <div class="container overflow-x-auto">
                <!-- SEARCH AND FILTER -->
                <div class="flex gap-6 w-full justify-between"> <!-- Added justify-between here -->
                    <!-- FILTERING -->
                    @php
                    $uniqueJournalNumbers = [];
                    @endphp
                    <div>
                        <form method="GET" action="{{ route('journals.index') }}" class="flex gap-4 lg:flex-row md:flex-row sm:flex-row ss:flex-row xs:flex-row flex-col"
                            id="filterForm">
                            <select id="sectionDropdown" name="sectionDropdown"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-[100%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                ...
                                <option value="{{ $student->section }}" {{ request('sectionDropdown') == $student->section ? 'selected' : '' }}>
                                    {{ $student->section }}</option>
                                @php
                            ...
                            <select id="dropdown" name="dropdown"
                                ...
                                <option value="{{ $uniqueJournalNumber }}" {{ request('dropdown') == $uniqueJournalNumber ? 'selected' : '' }}>
                                    Journal {{ $uniqueJournalNumber }}</option>
                                @endforeach
                            </select>

                            <select id="dropdown-status" name="status"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-[100%] p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                onchange="this.form.submit()">
                            ...
                            <button type="submit" name="reset" value="reset"
                                class="bg-[#AD974F] hover:bg-[#736023] text-white font-bold py-2 px-4 rounded-lg">Reset</button>
                        </form>
                    </div>
